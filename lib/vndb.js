/**
 ...
 | 	All Rights Reserved.                                                |
 +----------------------------------------------------------------------+
 | @Name: VNDB - Vu Ngo Database Helper                					                        |
 | @Author: VuNgo                					                    |
 | @Version: 1.0                					                    |
 | @Since: 2016-11                					                    |
 | @Source: github.com/ducvu91/node-react-socket-chat/blob/master/lib/vndb.js    |
...
var vnDB = {
    ...
    sqlQuery : function(sql){
        ...
        if(vnDB.usePromise)
            return vnDB.promsieQuery(sql);
        else
            return vnDB.query(sql);
    },

    query : function (sql){
        _writeLogQuery(sql);
        vnDB.pool.connect(function(err, client, done){
            if(err){
                done();
                ...
                client.query(sql, function(err, result){
                    ...
                    return vnDB.callback(err, result);
                });
            }
        });
    },

    promsieQuery : function(sql){
        return new Promise((resolve, reject) => {
            // reject and resolve are functions provided by the Promise
            // implementation. Call only one of them.
            // Do your logic here - you can do WTF you want.:)
            vnDB.pool.query(sql, (err, result) => {
     ...
     */
    sqlSelect : function (arrTable,cb){
     ...
     * @param arrData
     */
    sqlInsert : function (table,arrData,cb){
     ...
     * @param cb : function callback
     */
    sqlUpdate : function (table,arrData,sWhere,cb){
     ...
     * @param cb : function callback
     */
    sqlDelete : function (table,sWhere,cb){
...
function _writeLogQuery(data){
    ...
    var log = '';
    log += `${today.year}-${today.month}-${today.day}T${today.hour}:${today.minute}:${today.seconds} | ${today.timeZone} | ${status}` + '\n';
    log += message;
    log += `${data.sql}`+ '\n';
